...
export async function POST(req: NextRequest) {
  try {
    ...
    // Get user data
    const { data: userData } = await service.from('users').select('login_id, company_name').eq('id', user.id).single()
    ...
    if (!parentFolderId) {
      return NextResponse.json({ error: 'GOOGLE_DRIVE_PARENT_FOLDER_ID is not set' }, { status: 500 })
    }
    ...
    const fileName = `${templateName || 'design'}_${timestamp}.png`
    ...
    // Save request to DB
    const { data: requestData, error: reqError } = await service.from('image_requests').insert({
      user_id: user.id,
