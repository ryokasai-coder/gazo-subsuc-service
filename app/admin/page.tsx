      ...
      <header className="bg-white border-b border-[#EFEFEF] sticky top-0 z-40">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 h-[60px] flex items-center justify-between">
          <div className="flex items-center gap-2">
            <span className="w-7 h-7 rounded-lg bg-[#E85C97] flex items-center justify-center text-white font-bold text-xs">管</span>
            <span className="font-bold text-[#111111] text-sm">管理ダッシュボード</span>
          </div>
          <button onClick={handleLogout} className="text-sm text-[#6B7280] hover:text-[#111111] px-3 py-1.5 rounded-full hover:bg-[#F8F8FA] transition-all">ログアウト</button>
        </div>
      </header>

      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <div className="mb-6">
          <p className="text-xs font-bold text-[#E85C97] uppercase tracking-widest mb-1">Admin</p>
          <h1 className="text-2xl font-black text-[#111111]">管理ダッシュボード</h1>
          <p className="text-[#6B7280] text-sm mt-0.5">{billingMonth.replace('-', '年')}月 概要</p>
        </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-6">
              {[
                ...
                <div key={item.label} className="bg-white rounded-2xl p-4 shadow-sm text-center">
                  <div className={`text-2xl font-black ${item.red ? 'text-[#E85C97]' : 'text-[#111111]'}`}>{item.value}</div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {navItems.map(item => (
                ...
                >
                  ...
                  <div className="mt-4 flex items-center gap-1 text-xs text-[#E85C97] font-semibold">
                    開く
