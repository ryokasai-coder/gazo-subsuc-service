...
const STATUS_STYLES: Record<string, string> = {
  ...
  '未収': 'text-[#E85C97] bg-[#FFF0F6] ring-1 ring-red-200',
  '未処理': 'text-[#6B7280] bg-[#F8F8FA]',
}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-5">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center border-t-2 border-emerald-400">
            ...
            <div className="text-xs text-[#6B7280] mt-0.5">完了</div>
            <div className="text-sm font-semibold text-emerald-600">¥{totalCompleted.toLocaleString()}</div>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center border-t-2 border-[#E85C97]">
            <div className="text-2xl font-black text-[#E85C97]">{uncollected.length}件</div>
            <div className="text-xs text-[#6B7280] mt-0.5">未収</div>
            <div className="text-sm font-semibold text-[#E85C97]">¥{totalUncollected.toLocaleString()}</div>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center border-t-2 border-[#ABABAB]">
            ...
            <div className="text-xs text-[#6B7280] mt-0.5">未処理</div>
            <div className="text-sm font-semibold text-[#6B7280]">¥{totalUnprocessed.toLocaleString()}</div>
          </div>
          <div className="bg-white rounded-2xl p-4 shadow-sm text-center border-t-2 border-[#111111]">
            ...
            <div className="text-xs text-[#6B7280] mt-0.5">契約中顧客数</div>
        ...
        {uncollected.length > 0 && (
          <div className="bg-[#FFF0F6] border border-red-100 rounded-2xl p-4">
            <h3 className="font-bold text-[#E85C97] mb-2.5 text-sm">🚨 未収アラート（{uncollected.length}件）</h3>
            <div className="space-y-2">
              {uncollected.slice(0, 5).map(r => (
                <div key={r.id} className="flex items-center justify-between">
                  <span className="text-sm text-[#E85C97]">
                  ...
                  {r.user_id && (
                    <button onClick={() => sendDunning(r.user_id!, r.users?.email ?? '')}
                      className="text-xs bg-white text-[#E85C97] font-semibold px-3 py-1 rounded-full border border-red-200 hover:bg-red-50 transition-all">
                      督促メール送信
                    </button>
                  )}
                </div>
              ))}
              {uncollected.length > 5 && <p className="text-xs text-[#E85C97] opacity-70">他 {uncollected.length - 5} 件…</p>}
            </div>
          </div>
        )}
          ...
          {(['records', 'contracts'] as const).map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-5 py-2 text-sm font-semibold rounded-xl transition-all ${
                activeTab === tab ? 'bg-[#111111] text-white shadow-sm' : 'text-[#6B7280] hover:bg-[#F8F8FA]'
              }`}>
        ...
        {activeTab === 'records' && (
          <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
            <div className="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-[#F8F8FA]">
              ...
              <div className="ml-auto flex gap-2 flex-wrap">
                ...
                <label className="flex items-center gap-1.5 text-sm cursor-pointer text-[#6B7280]">
                  <input type="checkbox" checked={filterUnlinked} onChange={e => setFilterUnlinked(e.target.checked)} className="accent-[#E85C97] rounded" />
                  未紐付けのみ
                </label>
              </div>
            </div>
            <div className="overflow-x-auto">
                ...
                <tbody className="divide-y divide-[#F8F8FA]">
                  {records.map(rec => (
                    <tr key={rec.id} className="hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className={`${tdClass} font-mono text-xs text-[#6B7280]`}>{rec.billing_code}</td>
                      <td className={`${tdClass} text-[#6B7280] max-w-xs truncate`}>{rec.product_name}</td>
                      ...
                      <td className={tdClass}>
                        <span className={`text-xs px-2.5 py-1 rounded-full font-semibold ${STATUS_STYLES[rec.clearing_status] ?? 'text-[#6B7280] bg-[#F8F8FA]'}`}>
                        ...
                        {!rec.user_id && (
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => handleLink(rec.id, rec.billing_code)}
                              className="text-xs bg-[#FFF0F6] text-[#E85C97] font-semibold px-2.5 py-1 rounded-full hover:bg-red-100 transition-all">
                              保存
                            </button>
                          </div>
                        )}
              ...
              {loading && <div className="flex justify-center py-8"><div className="w-6 h-6 border-2 border-[#E85C97] border-t-transparent rounded-full animate-spin" /></div>}
        ...
        {activeTab === 'contracts' && (
          <div className="bg-white rounded-3xl shadow-sm overflow-hidden">
            <div className="px-5 py-4 border-b border-[#F8F8FA]">
                ...
                <tbody className="divide-y divide-[#F8F8FA]">
                  {contracts.map(con => (
                    <tr key={con.id} className="hover:bg-[#FAFAFA] transition-colors">
                      ...
                      <td className={`${tdClass} font-mono text-xs text-[#6B7280]`}>{con.billing_code}</td>
                      <td className={`${tdClass} text-[#6B7280]`}>{con.product_name}</td>
