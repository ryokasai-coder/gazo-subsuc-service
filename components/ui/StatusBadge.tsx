...
const statusConfig: Record<Status, { label: string; className: string }> = {
  ...
  cancelled:   { label: 'キャンセル', className: 'bg-[#F8F8FA] text-[#6B7280]' },
  ...
  overdue:     { label: '滞納',     className: 'bg-[#FFF0F6] text-[#E85C97] ring-1 ring-red-200' },
}

export default function StatusBadge({ status }: { status: string }) {
  const config = statusConfig[status as Status] ?? { label: status, className: 'bg-[#F8F8FA] text-[#6B7280]' }
  return (
